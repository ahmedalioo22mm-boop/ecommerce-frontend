...
const LoginPage = () => {
  const { login } = useAuth(); // 2. استخدام دالة login من الـ Context
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 3. استدعاء دالة login من الـ Context
      ...
      if (response.success) {
        ...
        setTimeout(() => {
          router.push("/");
        }, 1500); // تقليل مدة الانتظار
      } else {
        // إذا فشل الطلب ولكن الخادم أرسل رسالة خطأ
        setError(
          response.message || "Login failed. Please check your credentials."
        );
      }
    } catch (err) {
      // إذا حدث خطأ في الشبكة أو خطأ غير متوقع
